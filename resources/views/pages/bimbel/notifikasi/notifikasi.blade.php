                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            @foreach($notif as $notif)
                                <tr>
                                    ...
                                    <td>{{$notif->name}}</td>
                                    <td>{{$notif->email}}</td>
                                    <td><img src="{{asset('uploads/user siswa/'.$notif->avatar)}}" width="50"
                                             height="50"></td>
                                    @if($notif->status == '0')
                                    ...
                                    <td>
                                        @if($notif->status != '0')
                                            <form action="{{route('notifikasi.bimbel.diterima', $notif->id)}}"
                                                  method="put">
                                                <button type="submit" class="btn btn-sm btn-success">Diterima</button>
                                            </form>
                                        @else
                                            <a class='btn btn-danger btn-sm' href="{{route('usiswa.destroy', $notif->id)}}"
                                               onclick="return confirm('Apakah Anda Yakin Akan Menghapus Da ta Ini?')" class=""><i
                                                        class="fa fa-trash"></i> </a>
                                        @endif

                                        @if($notif->status != '0')
                                            <form action="{{route('notifikasi.bimbel.ditolak', $notif->id)}}"
                                                  method="put">
